...
\begin{singlespace}
\title{{\bf {\Large Adaptation of a weighted regression approach to evaluate water quality trends in Tampa Bay, Florida}}}
\author{
...
Accelerated rates of eutrophication have been documented in aquatic systems worldwide and is of particular concern for coastal waters that support numerous aquatic life and human uses.  Eutrophication is broadly defined as an increase in the rate of supply of organic matter to a system \citep{Nixon95}, typically caused by elevated nitrogen or phosphorus loads.  Although nutrients are necessary for the growth of primary producers (i.e., macrophytes, algae), excessive anthropogenic inputs beyond background concentrations can have serious consequences for the structure and function of aquatic systems.  Eutrophication of coastal systems has been associated with depletion of dissolved oxygen from the decomposition of organic matter \citep{Diaz08}, increases in the frequency and severity of harmful algal blooms \citep{Glibert13}, and reduction or extirpation of seagrass communities \citep{Duarte95,Tomasko05}.  System-wide changes can occur as the effects of eutrophication on primary production may influence upper trophic levels \citep{Powers05}. 

The effects of eutrophication are generally well understood, particularly for freshwater systems. The consequences of nutrient pollution were increasingly obvious by the 1960s such that eutrophication became a central focus of limnological research \citep{Cloern01}.  However, the importance of understanding the relative effects of eutrophication on coastal systems were not realized until several decades later.  For example, \citet{Rosenberg85} described the future hazards of coastal eutrophication nearly twenty years after similar issues were the focus of intense study in freshwater systems.  The conceptual understanding of nutrient dynamics in coastal systems has, therefore, been limited to simple extrapolations of freshwater eutrophication models.  \citet{Cloern01} suggests that system-specific attributes modulate coastal response to nutrient inputs, such that more appropriate conceptual models that recognize linked changes in relevant state variables are needed.  To date, empirical models that are flexible and appropriate for site-specific conditions have not been extensively applied to describe nutrient-response dynamics in estuaries.

The increasing availability of long-term, high resolution datasets has further underscored the need to develop and apply quantitative nutrient-response models for coastal systems.  In many cases \citep[e.g.,][]{Greening06}, long-term datasets have sufficiently described general trends in response to changing nutrient regimes, although unambiguous and quantitative descriptions of direct and indirect responses have been lacking.  For example, temporal variation in phytoplankton growth dynamics are often apparent by season with typical late summer blooms in temperate or tropical systems \citep{Cloern96}, whereas climate variation may contribute to substantial deviation in growth patterns between years \citep{Jassby02}.  Additionally, spatial heterogeneity in algal response to nutrients is common across salinity gradients such that effects of flux variation are most apparent near freshwater inflows \citep{Cloern96}.  Simple statistical models that are constrained by assumptions of linearity and stationarity  of variables through time may not adequately characterize subtleties in the variation of nutrient-response measures at different scales.  Novel techniques that leverage the descriptive capabilities of large datasets are needed to improve our understanding of temporal and spatial variation in chlorophyll dynamics as a measure of eutrophication.

The use of simple descriptive statistics to evaluate the outcomes of management efforts may be ill-advised given that general trends in monitoring data may be more reflective of natural variation in system characteristics.  \citet{Hirsch10} developed the \ac{WRTDS} approach to model pollutant concentration in rivers to address these issues and shortcomings of previously-developed models.  \ac{WRTDS} provides a flexible interpretation of water quality changes by estimating multiple parameters that are specific to a given season, year, and discharge for individual observations across the time series.  This allows for a more informed assessment of water quality changes than standard regression models that characterize trends using a single set of parameters.  Accordingly, the approach addresses the need to focus on descriptions of change in relation to changes among water quality variables across time, rather than a specific focus on hypothesis testing. The approach has been applied to model pollutant delivery from tributary sources to Chesapeake Bay \citep{Hirsch10,Moyer12,Zhang13}, Lake Champlain \citep{Medalie12}, and the Mississippi River \citep{Sprague11}.  Improvements in the description of water quality trends suggest the approach could be applied directly to estuaries to characterize and better understand drivers of primary production.  Moreover, changes in pollutant sources and variation in freshwater inputs over time for many coastal systems warrant the use of novel methods for trend evaluation.

The goal of the analysis was to resolve factors in estuaries that influence algal growth dynamics by considering the effects of time, season, and freshwater inputs.  We adapted the \ac{WRTDS} approach developed by \citet{Hirsch10} to create an accurate representation of water quality trends using a multi-decadal dataset from Tampa Bay, Florida. The analysis included four main objectives.  First, we described the weighted regression model developed by \citet{Hirsch10} and provided a rationale for its adaptation to estuaries.  Second, the model was applied to the time series in different segments of Tampa Bay to characterize and evaluate overall trends in the mean response of \ac{chl} as well as the frequency of occurrence of extreme events using quantile regression with the \ac{WRTDS} model.  Third, additional factors related to water quality were used to describe the unexplained variance in \ac{chl} growth patterns not characterized by the model.  Finally, we developed informed hypotheses to explain temporal and spatial patterns of \ac{chl} growth in response to large scale drivers that accounts for the natural variability in water quality.  Results from the analysis will provide a natural history of water quality changes in Tampa Bay that is temporally consistent with drivers of change.  In general, such techniques may be infuential for improving our understanding of the nutrient-response paradigm in coastal systems.  

\section{Methods}

\subsection{Study area}

Tampa Bay is the largest estuary in Florida and the second largest in the Gulf of Mexico (\cref{fig:tb_map}).  The Bay has a surface area of 1000 km$^2$, approximate water volume of 3.2 km$^3$, drains a 5890 km$^2$ watershed, and is generally shallow with a mean depth of 3.3 m \citep{Goodwin87,Wang99}. Four major segments are defined by geopolitical boundaries and natural gradients (\cref{fig:tb_map,tab:segsum}; \citealt{Lewis85}): Hillsborough Bay, Old Tampa Bay, Middle Tampa Bay, and Lower Tampa Bay.  A majority of the freshwater inflow to the Bay comes from four sources with a combined average discharge of 53.8 m$^3 \cdot$ s$^{-1}$.  The Hillsborough and Alafia rivers drain into Hillsborough Bay, Manatee river drains into Lower Tampa Bay, and Little Manatee river drains into Middle Tampa Bay.  Old Tampa Bay receives freshwater inputs from Lake Tarpon to the north.  Hundreds of tributaries contribute the remainder of freshwater inputs, in addition to atmospheric, groundwater, and industrial/municipal contributions. The watershed is extensively developed with 42\% urban cover and 17\% agricultural land use, whereas wetlands and forests account for 18\% of the watershed  \citepalias{TBEP11}. Population density has increased dramatically in urban areas with increases of 148\% and 158\% in Pinellas and Hillsborough counties from 1960-2001 \citep{Xian07}. Total population of the watershed in 2010 was approximately 2.7 million individuals \citepalias{TBEP11}.

The climate is sub-tropical with a mean annual temperature of 22.3$^{\circ}$C, ranging from 16.0$^{\circ}$ in January to 27.8$^{\circ}$ in August \citep{Wooten85}. Mean annual precipitation is 123.7 cm \citep{Wooten85}, whereas \ac{ENSO} events have contributed to inter-annual variation of precipitation \citep{Schmidt02}.  Warmer (El Ni\~{n}o) years are characterized by increases in freshwater inputs in fall and winter, whereas cooler (La Ni\~{n}a) years are associated with decreases in freshwater inputs in winter and spring.  \ac{ENSO} variation has been asssociated with seasonal changes in longitudinal salinity profiles for the Bay \citep{Schmidt02}.  Seasonal variation in precipitation not associated with \ac{ENSO} is also observed, such that maximum rainfall occurs during July and August from convectional heating of land surfaces in the late afternoon.  Secondary seasonal peaks in precipitation are also observed in February and March \citep{Schmidt02}.  Tidal influences are primarily limited to Middle and Lower Tampa Bay with salinity continuously above 25 \ac{ppt}. \citet{Goodwin87} estimated tidal exchange rates of total volume for Old Tampa and Hillsborough Bays at 4.6\% and 1.4\%, respectively. Overall, Tampa Bay is vertically well mixed and typically does not stratify \citep{Lewis88}.

\subsection{Observed water quality trends in Tampa Bay} \label{tb_wq}

Approximate nitrogen loads in the mid 1970s were estimated at $8.2 \times 10^6$ kg yr$^{-1}$, with approximately $5.5 \times 10^6$ kg yr$^{-1}$ entering Hillsborough Bay alone \citep{Poe05,Greening06}.  Reduced water clarity associated with phytoplankton biomass contributed to dramatic reduction in the areal coverage of seagrass \citep{Tomasko05} and development of hypoxic events causing a decline in benthic faunal production \citep{Santos80}.  Municipal wastewater contributed 55\% of nitrogen loads, with the remainder attributed to non-point sources, atmospheric deposition, groundwater, and losses from fertilizer distribution facilities at ports \citep{Poe05}.  Extensive efforts to reduce nutrient loads to the Bay occurred by the late 1970s, with the most notable being improvements in infrastructure for wastewater treatment in 1979.  Improvements in water clarity and decreases in \ac{chl} were observed bay-wide in the 1980s, with conditions generally remaining constant to present day.  Non-point sources are currently considered the primary inputs of nutrients \citep{Pribble01,Poe05}.  The establishment of annual load targets has been a recent management priority for the restoration of seagrass to historical coverage \citep{Tomasko05,Greening06}.  

Annual variation in \ac{chl} from the 1980s to present has been observed, although mean concentrations have been lower than 1970s averages (\cref{fig:obsyrmo1}).  Annual variation in \ac{chl} may be related to salinity \citep{Schmidt02}, climate \citep{Wang99,Schmidt02}, and seagrass growth \citep{Greening06}, in addition to human-induced changes.  Many of these relationships have not been explicitly described for Tampa Bay given confounding effects between multiple drivers on estuary production. For example, the relationship of \ac{chl} concentration with salinity differs based on the period of evaluation (\cref{fig:salmoyr1}).  Chlorophyll shows a negative correlation with salinity from 1980 to present, whereas the relationship in years prior is less obvious, suggesting changes in drivers of primary production following management actions.  Moreover, trends in \ac{chl} concentration vary by season following rainfall patterns such that peaks are observed in the late summer months (\cref{fig:salmoyr2,fig:obsyrmo2}).  Seasonal trends may also vary by year given annual climate variation \citep{Schmidt02}.

\subsection{Data}

Water quality data for Tampa Bay have been collected since the early 1970s. We compiled a time series of \ac{chl} concentration (\mugl) using data from the Hillsborough County \ac{EPC} \citepalias{TBEP11}.   Monthly water quality data were obained for mid-depth at 50 stations throughout the Bay (\cref{fig:tb_map}) from 1974 to 2012, producing approximately 456 observations per station ($n=$ \Sexpr{nrow(tb.dat)}).  Stations were visited on a rotating schedule whereby one third of all stations were sampled each week.  Bay segments represent management units of interest with distinct chemical and physical differences \citep[\cref{tab:segsum},][]{Lewis85}.  Accordingly, station data were averaged by segment.  In addition to \ac{chl}, salinity data were obtained and used as a tracer of freshwater influence on water quality.  We expected that salinity was an important factor influencing interpretation of \ac{chl} trends relative to the effects of additional factors (e.g., date, load, seagrass, etc.).  Salinity data were converted to dimensionless values that represent the fraction of freshwater \citep{Dyer73} from 0--1, such that:
\begin{equation}
...
The \ac{WRTDS} approach developed by \citet{Hirsch10} was adapted to interpret trends in \ac{chl} in Tampa Bay using 39 years of data.  The functional form of the \ac{WRTDS} approach is a regression model that relates pollutant $\ln$-concentration to discharge and time.  This model was adapted to relate chlorophyll concentration to salinity and time:
...
Observed \ac{chl} for all dates indicated mean values decreasing from Hillsborough (\Sexpr{chl.mean.o[['HB']]} \mugl), to Old (\Sexpr{chl.mean.o[['OTB']]}), to Middle (\Sexpr{chl.mean.o[['MTB']]}), and to Lower Tampa Bay (\Sexpr{chl.mean.o[['LTB']]}).  Observed trends from 1974 to 2012 indicated a consistent decrease from 1974 to present as previously documented, with the most dramatic declines observed in the 1980s following wastewater treatment efforts (\cref{fig:obsyrmo1}).  Percent decrease by segment based on mean concentration of \ac{chl} for observations prior to and after January 1\textsuperscript{st} 1980 was \Sexpr{chl.per.o[['HB']]}\% for Hillsborough, \Sexpr{chl.per.o[['MTB']]}\% for Middle,  \Sexpr{chl.per.o[['OTB']]}\% for Old, and \Sexpr{chl.per.o[['LTB']]}\% for Lower Tampa Bay.  Observed trends in \ac{chl} before and after treatment for the entire Bay indicated a decrease of \Sexpr{chl.per.o2}\%. Interestingly, the most dramatic declines in \ac{chl} following treatment of point sources of pollution were not observed until a few years after 1979.  For example, hypereutrophic conditions persisted in Hillsborough Bay until 1984 when mean annual concentration from 1983 to 1984 decreased by \Sexpr{hdrop(1983,1984)}\%, whereas a decrease of only \Sexpr{hdrop(1979,1980)}\% was observed from 1979 to 1980.  Peaks in \ac{chl} have also been observed in the mid-1990s associated with El Ni\~{n}o effects \citep{Greening06}.  For example, \Sexpr{chl.nino.o[['OTB']]$Chla_ugl} \mugl of \ac{chl} was observed for Old Tampa Bay in \Sexpr{chl.nino.o[['OTB']]$month.name} \Sexpr{chl.nino.o[['OTB']]$year}.  More extreme observations have been observed for individual stations during El Ni\~{n}o events. 
...
#merge mean values w/ slope estimates
mod.dat<-merge(mean.vals,mod.dat,by=c('seg','time.cat','variable'))
names(mod.dat)[names(mod.dat)=='variable']<-'model'

#format p values and combine with slope values
symp<-function(pval) symnum(pval,corr=FALSE, #from sympnum
               cutpoints=c(0,.001,0.01,.05, 1),
               symbols=c("***","**","*"," "))

dig.val<-2
p.sym<-symp(mod.dat[,'Pr(>|t|)'])
mod.dat$Estimate<-paste0(format(round(mod.dat$Estimate,2),nsmall=dig.val),p.sym)

mod.dat$V1<-format(round(mod.dat$V1,dig.val),nsmall=dig.val)

mod.dat<-mod.dat[,!names(mod.dat) %in% 'Pr(>|t|)']
names(mod.dat)[names(mod.dat) %in% 'V1']<-'mean'

#melt and recast for table
mod.dat<-melt(mod.dat,measure.vars=c('mean','Estimate'))
mod.dat<-data.frame(recast(mod.dat,seg+model~time.cat+variable))

mods<-factor(mod.dat$model,labels=c('mean','0.9 $\\tau$','0.1 $\\tau$'))
segs<-levels(mod.dat$seg)
mod.dat<-mod.dat[,-c(1:2)]

cap.val<-'Decadal summaries of salinity-normalized \\ac{chl} (\\mugl) trends by Bay segment. Trends are evaluated for models fit through the mean response and the 10\\textsuperscript{th} and 90\\textsuperscript{th} percentile ($\\tau$) distributions.  Mean and slope estimates are aggregated by year categories using monthly data.'
...
cap.val<-'Variation in \\ac{chl} (\\mugl) for Bay segments by year and season categories. \\ac{CV} values are shown for salinity-normalized predictions.  Trends are evaluated for models fit through the mean response and the 10\\textsuperscript{th} and 90\\textsuperscript{th} percentile ($\\tau$) distributions.'
...
%chl trends by salinity, year, and month
<<salmoyr,cache=T,include=F>>=
#changing trends by year, month, salinity

ylabs<-expression(paste('Chloropyhll-',italic(a),' (',italic('\u03bc'),'g ',L^-1,')'))

to.plo<-epc.est[epc.est$seg=='HB',]
...
p1<-ggplot(to.plo,aes(x=Salinity_ppt,y=exp(Chla_ugl))) + 
  ...
  scale_colour_manual('Wastewater treatment',values=c('black','grey')) +
  scale_shape_manual('Wastewater treatment',values=c(16,17))

#chlorophyll a over time, combo of year and month plots
p2<-ggplot(to.plo[to.plo$year %in% c('1975','1985','1995','2005'),],aes(x=month.name,y=exp(Chla_ugl),group=year,colour=year)) + 
  ...
  scale_colour_manual('Year',values=c('grey80','grey53','grey27','grey0'))

# grid.arrange(arrangeGrob(p1,p2,nrow=2,heights=c(9,10),left=textGrob(ylabs,rot=90)))

pdf('M:/docs/manuscripts/tb_chl/figs/salmoyr.pdf',height=4,width=4.5,family='serif')
print(p1)
print(p2)
dev.off()
@
...
\centering
\subfloat[]{
\includegraphics[width=0.6\textwidth,page=1,trim=0in 0in 0in 0.25in,clip]{M:/docs/manuscripts/tb_chl/figs/salmoyr.pdf}
\label{fig:salmoyr1}
}

\subfloat[]{
\includegraphics[width=0.6\textwidth,page=2,trim=0in 0in 0in 0.25in,clip]{M:/docs/manuscripts/tb_chl/figs/salmoyr.pdf}
\label{fig:salmoyr2}
}

\leavevmode\smash{\makebox[0pt]{\hspace{15em}% HORIZONTAL POSITION           
  \rotatebox[origin=l]{90}{\hspace{16em}% VERTICAL POSITION
    Chlorophyll-\textit{a} (\mugl)}%
}}\hspace{0pt plus 1filll}\null

\caption{Variation in \ac{chl} by \protect\subref{fig:salmoyr1} salinity, \protect\subref{fig:salmoyr2} season, and \protect\subref{fig:salmoyr2} year in Hillsborough Bay.  Panel \protect\subref{fig:salmoyr1} shows the relationship between salinity and \ac{chl} before and after extensive wastewater treatment in 1979.}
\label{fig:salmoyr}
\end{figure}
\captionsetup[subfloat]{position=top}

%weighting example
<<wtex,cache=T,fig.height=8.5,fig.width=7,out.width= "6in",fig.cap='Example of weighting for one observation in Hillsborough Bay.  The top plot shows all data weighted for October 2003 when the proportion freshwater was 0.39.  Point size and color are in proportion to weights.  The bottom plots show the individual weights for month, year, proportion freshwater, and all weights combined.'>>=
...
p3<-ggplot(p3.dat,aes(x=Date,y=Wt)) + 
  ...
  scale_y_continuous(name=element_blank(),limits=c(0,1)) +
  scale_x_date(name=element_blank(),limits=as.Date(c("2000-01-01", "2005-01-01"))) +
  ggtitle(titles[[3]]) +
  theme_bw()
...
<<predobs,cache=T,fig.height=5,fig.width=8.5,out.width= "9in",fig.cap='Predicted and observed \\ac{chl} concentrations for Tampa Bay segments. Predicted values are for the weighted regression models fit through the mean response. HB: Hillsborough Bay, OTB: Old Tampa Bay, MTB: Middle Tampa Bay, LTB: Lower Tampa Bay.'>>=

#y axis label for plots
ylabs<-expression(paste('Chloropyhll-',italic(a),' (',italic('\u03bc'),'g ',L^-1,')'))

ggplot(epc.est,aes(x=dec.time)) + 
  geom_point(aes(y=exp(Chla_ugl),shape='Obs'),colour='gray50',alpha=0.9) +
  geom_line(aes(y=bt.md,linetype='Pred',colour='Pred'),alpha=0.9) +
...
\begin{landscape}
\centering\vspace*{\fill}
<<salnrm2,cache=T,fig.height=5,fig.width=8.5,out.width="9in",fig.cap='Salinity-normalized results for the mean response and the 10\\textsuperscript{th} and 90\\textsuperscript{th} quantile ($\\tau$) distributions. HB: Hillsborough Bay, OTB: Old Tampa Bay, MTB: Middle Tampa Bay, LTB: Lower Tampa Bay.'>>=
est<-sal.nrm
...
ggplot(est,aes(x=as.numeric(dec.time),group=seg)) + 
  geom_point(aes(y=exp(Chla_ugl),shape='Obs'),alpha=0.9,
    colour='gray70') +
...
est<-melt(est,measure.vars=c('b.md','b.hi','b.lo'))
est$variable<-factor(est$variable,levels=levels(est$variable),labels=c('Mean','q0.9','q0.1'))

#increase is beta vals multiplied by sal.ref to get back into units of log chl
#direct exponentiation of beta is fucked
est$incr<-est$sal.ref*est$value

to.plo<-est[est$variable=='Mean',]

#year
ggplot(to.plo,aes(x=sal.ref)) + 
  geom_point(aes(y=incr),alpha=0.7,size=2.5,colour='gray60') +
  stat_smooth(aes(y=incr),se=F,size=1,method='loess',colour='gray10') + 
  scale_y_continuous(expression(paste(Delta,' ln(Chlorophyll-',italic(a),')'))
    ) +
  theme_bw() +
  facet_grid(seg~yr.cat) +
