...
for(seg in segs){
  ...
  for(row in 1:nrow(dat.in)){
    ...
    out.nrm<-grd.in[grd.mtch,c('bt.lo','bt.md','bt.hi')]
...
sal.nrm.all<-fit
names(sal.nrm.all)[(ncol(sal.nrm.all)-2):ncol(sal.nrm.all)]<-c('norm.lo','norm.md','norm.hi')

sal.nrm<-aggregate(cbind(norm.lo,norm.md,norm.hi)~dec.time+seg,
  mean,data=sal.nrm.all)

sal.nrm<-merge(epc.est,sal.nrm,by=c('dec.time','seg'))

save(sal.nrm,file='data/sal_nrm.RData')
