...
# colors
cols <- brewer.pal(11, 'Spectral')[c(10, 1)]

p1 <- ggplot(to.plo,aes(x=dec.time, shape = month.name, group = month.name, colour = month.name)) + 
  geom_point(aes(y=exp(Chla_ugl)),
    alpha=0.9) +
...
ggplot(epc.est[sel.vec,],aes(x=date.f)) + 
  geom_point(aes(y=exp(Chla_ugl),shape='Obs'),alpha=0.9,colour=cols[2]) +
...
ggplot(to.plo,aes(x=as.numeric(year))) + 
  ...
  geom_point(data=est,aes(y=bt.lo,colour='q3',shape='q3'),
    alpha=0.9) +
  geom_line(aes(y=norm.lo,linetype='q3'),colour=cols[3],alpha=0.9) +
  scale_y_continuous(ylabs) +
  theme_bw() +
  ...
  guides(linetype=guide_legend(
    override.aes=list(colour=c(cols[1], cols[2],cols[3]))
    )) +
  scale_shape_manual(
    name='Predicted',
    labels=c(
...
ggplot(to.plo,aes(x=as.numeric(dec.time),group=seg)) + 
  ...
  geom_line(aes(y=norm.lo,linetype='q3',colour='q3'),alpha=0.9) +
  scale_y_continuous(ylabs) +
  theme_bw() +
  facet_wrap(~seg,scales='free_y') +
  scale_x_continuous(
  ...
  scale_colour_manual(
    ...
    values=c('q1'=cols[1],'q2'=cols[2],'q3'=cols[3])
    ) 
@
