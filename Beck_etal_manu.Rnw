...
p1<-ggplot(to.plo,aes(x=year,y=Chla_ugl)) + 
...
p2<-ggplot(to.plo,aes(x=month.name,y=Chla_ugl)) + 
...
# y axis label for plots
ylabs<-expression(paste('Chloropyhll-',italic(a),' (',italic('\u03bc'),'g ',L^-1,')'))
p1 <- ggplot(to.plo,aes(x=dec.time)) + 
  geom_point(aes(y=exp(Chla_ugl)), shape = 16,
